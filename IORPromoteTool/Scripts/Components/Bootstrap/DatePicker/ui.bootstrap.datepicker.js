...
.controller('DatepickerController', ['$scope', '$attrs', '$parse', '$interpolate', '$log', 'dateFilter', 'datepickerConfig', function($scope, $attrs, $parse, $interpolate, $log, dateFilter, datepickerConfig) {
  ...
  $scope.select = function( date ) {
    if ( $scope.datepickerMode === self.minMode ) {
      ...
      dt.setHours(17); // Mike: Added so that date saved to UTC will be correct date. Otherwise local time will cause date saved to be one day before date displayed.
...
.directive('daypicker', ['dateFilter', function (dateFilter) {
  return {
    restrict: 'EA',
    replace: true,
    templateUrl: 'PartialViews/datepicker/day.html',
    require: '^datepicker',
...
.directive('monthpicker', ['dateFilter', function (dateFilter) {
  return {
    restrict: 'EA',
    replace: true,
    templateUrl: 'PartialViews/datepicker/month.html',
    require: '^datepicker',
    link: function(scope, element, attrs, ctrl) {
      ctrl.mode = {
        step: { years: 1 },
...
.directive('yearpicker', ['dateFilter', function (dateFilter) {
  return {
    restrict: 'EA',
    replace: true,
    templateUrl: 'PartialViews/datepicker/year.html',
    require: '^datepicker',
    link: function(scope, element, attrs, ctrl) {
      ctrl.mode = {
...
.directive( 'datepicker', function () {
  return {
    restrict: 'EA',
    replace: true,
    templateUrl: 'PartialViews/datepicker/datepicker.html',
    scope: {
      datepickerMode: '=?',
      dateDisabled: '&'
    },
        ...
        if (date === 'today') {
          ...
          if (angular.isDate(ngModel.$modelValue)) {
            ...
            date.setFullYear(today.getFullYear(), today.getMonth(), today.getDate());
              ...
              date = new Date(today.setHours(17, 0, 0, 0)); // Mike: Added so that date saved to UTC will be correct date. Otherwise local time will cause date saved to be one day before date displayed.
          }
        }
...
.directive('datepickerPopupWrap', function() {
  return {
    ...
    replace: true,
    transclude: true,
    templateUrl: 'PartialViews/datepicker/popup.html',
